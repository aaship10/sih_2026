...
def world_velocity_to_ego(velocity_world, ego_yaw_deg):
    """
    Converts a CARLA world-frame velocity vector into the project's
    ego-frame convention:

        x = forward
        y = left

    velocity_world: [vx, vy, vz] in CARLA world coordinates
    ego_yaw_deg: ego vehicle yaw in CARLA degrees

    Returns:
        [vx_ego, vy_ego, vz_ego]
    """
    vx_world, vy_world, vz_world = velocity_world
